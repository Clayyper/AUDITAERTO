...
function createRelatorioBuffer(payload) {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fillColor('#0f172a').font('Helvetica-Bold').fontSize(14).text(calculo?.entrada?.nome || 'Não informado', 55, 128);
      doc.font('Helvetica').fontSize(10).fillColor('#334155').text(`Admissão: ${calculo?.entrada?.dataAdmissao || campos.dataAdmissao || '—'}`, 320, 114);
      doc.text(`Desligamento: ${calculo?.entrada?.dataDemissao || campos.dataDesligamento || '—'}`, 320, 130);
      doc.text(`Salário base: ${money(calculo?.entrada?.salarioBase ?? campos.salarioBase)}`, 320, 146);
