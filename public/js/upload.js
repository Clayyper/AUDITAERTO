...
function buildFriendlyStatus(data) {
  const lines = [];
  lines.push(`Arquivo: <strong>${escapeHtml(data.arquivo.nome)}</strong>`);
...
uploadForm.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    trctImportado = data;
    window.AppState?.saveState?.({ trctImportado: data });
...
gerarPdfBtn.addEventListener('click', async () => {
  ...
  if (!response.ok) {
    uploadStatus.innerHTML = '<div class="status-warn">Não foi possível gerar o PDF agora.</div>';
    return;
  }
